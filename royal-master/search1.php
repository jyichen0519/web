...
<body>
    ...
    <center>
   <table class="table table-hover" width="1500" align=center>
     <thead class="ListCap"><td colspan=8>預約紀錄</td></thead>
   
     <tr><th>no</th><th>預約者姓名</th><th>預約類別</th><th>教室</th<th>時間</th><th>日期</th><th>學期</th><th>星期</th><th>操作</th></tr>;
    <?php
     // 使用 mysqli_fetch_assoc() 將 $result 裡的資料取出
     while($row = mysqli_fetch_assoc($result)) {
        echo "<tr>";
        echo "<td>" . reservation($row["reservation_id"]) . "</td>";
        echo "<td>" . reservation($row["name"]) . "</td>";
        echo "<td>" . reservation($row["type"]) . "</td>";
        echo "<td>" . reservation($row["classroom"]) . "</td>";
        echo "<td>" . reservation($row["during"]) . "</td>";
        echo "<td>" . reservation($row["date"]) . "</td>";
        echo "<td>" . reservation($row["semester"]) . "</td>";
        echo "<td>" . reservation($row["week"]) . "</td>";
